import { setAudio, getAUDIO } from "./fetchMusic.js";
import { sendToPlayerTrack } from "./player.js";

const AMOUNT_TRACK = 18; // The amount track in the start
  ...
  try {
    ...
    musicContainer.addEventListener('click', async (event) => {
      ...
      if (link) {
        ...
        const trackInfo = JSON.parse(link.getAttribute('data-track'));
        console.log(trackInfo);
        sendToPlayerTrack(trackInfo);
        try {
          const audioPath = await getAUDIO(trackInfo.title);
